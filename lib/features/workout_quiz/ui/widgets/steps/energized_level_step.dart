...
class EnergizedLevelStep extends StatelessWidget {
  const EnergizedLevelStep({super.key});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        QuizTitleSection(
          title: t.workout_quiz.steps.energized_level.title,
          subtitle: t.workout_quiz.steps.energized_level.subtitle,
        ),
        const SizedBox(height: 32),
        BlocSelector<WorkoutQuizCubit, WorkoutQuizState, EnergizedLevel?>(
          selector: (state) => state.energizedLevel,
          builder: (context, value) {
            final cubit = context.read<WorkoutQuizCubit>();
            return Column(
              children: EnergizedLevel.values.map((level) {
                return Padding(
                  padding: const EdgeInsets.only(bottom: 12),
                  child: RadioButtonOption(
                    title: level.title(t),
                    isSelected: value == level,
                    onTap: () => cubit.setEnergizedLevel(level),
                  ),
                );
              }).toList(),
            );
